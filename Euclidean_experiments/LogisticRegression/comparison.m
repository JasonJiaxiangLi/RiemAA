...
%for i=1:length(prob_name)
        ...
        [x, time_AA2, error_AA2,error_AA2_g,index_AA2_fun] =AA_modified(f,gradient, x0, c, stepSize,N,andersonm,1);
...
optimal_f=min([min(error_LM_AA1),min(error_AA2),min(error_rna5),min(error_rna10),min(error_rna20)]);
error_gd=(error_gd-optimal_f)/optimal_f;       
error_LM_AA1=(error_LM_AA1-optimal_f)/optimal_f; 
error_AA2=(error_AA2-optimal_f)/optimal_f;
...
hold on
legend({'Gradient Descend', 'LM-AA', 'New-AA','RNA k=5','RNA k=10','RNA k=20'},'location','SW')
xlabel('time(s)');
ylabel('(F-F*)/F*');
...
hold on
semilogy(1:length(error_AA2),error_AA2,'m');
hold on
